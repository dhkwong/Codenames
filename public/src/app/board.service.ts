...
export class BoardService {
  ...
  createBoard() {
    ...
    for (let num = 0; num < 8; num++) {
      ...
      this.board[randBoard] = { 1: words[randIndex] }
    ...
    for (let num = 8; num < 17; num++) {
      let randIndex = numbers[num];
      let randBoard = boardnums[num]
      this.board[randBoard] = { 2: words[randIndex] }
    }
    //Assigning for YELLOW cards
    for (let num = 17; num < 24; num++) {
      let randIndex = numbers[num];
      let randBoard = boardnums[num]
      this.board[randBoard] = { 0: words[randIndex] }
    }
    //Assigning for Assassin/Black card 
    this.board[boardnums[24]] = { 3: words[numbers[24]] }
    console.log("board.service board: "+JSON.stringify(this.board));
    return this.board
  }
